...
参考文献：最经典最地道的 [Regev](https://cims.nyu.edu/~regev/teaching/lattices_fall_2009/) 讲义（源于它，但不仅仅是它）
...
如Figure 2阴影部分所示，就是fundamental parallelepiped的示例。假如把维度空间内所有的**格点**都做一个 $\mathcal{P}(B)$, 就能平铺(tiling)掉整个维度空间(span)。一定要注意： $\Lambda$ 不能表示整个n维空间， $span(\Lambda)$ 才能表示整个n维空间。区别就在于span的系数是实数，而格基的系数只能是整数，所以格只能表示一群离散的格点。
...
不难看出，虽然格可以刻画 n 维空间离散点集，但是格的行列式，也就是上面提到的「基础区域」的容量volumn。因此，当我们以后提到格的容量时，无特别说明，都是指基础区域的容量。
...
如上图， $b_1,b_2$是一组线性无关向量，它可以作为二位平面的一个格基。以一个向量为基准，不妨设 $\tilde{b}_1=b_1$，利用投影公式可以求另一个向量 $b_2$ 在这个向量的投影： $\tilde{b}_2=b_2-\frac{(\tilde{b}_1,b_2)}{(\tilde{b}_1,\tilde{b}_1)}\tilde{b}_1$ . 
...
- 正交后的格基无需原格基所形成的格的等价基，甚至，它们一般不在一个格内，见Figure 4.
...
断言 1：如果一个格 $\Lambda$ 最小基向量长度存在，那么格中一定存在某个 $\pmb{v}_i$ 满足 $||\pmb{v}i||=\lambda_i(\Lambda)$ 
...
这三个变种，Search SVP、Optimization SVP和Decisional SVP的难度以此下降，但是反之依然。因此，这三个难题是等价的。
...
- Optimization  $CVP_\gamma$ ：给定格基 $B\in \mathbb{Z}^{m\times n}$ 和 向量 $\pmb{t}\in\mathbb{Z}^m$ ，找到距离 d 满足  $d\le dist(t,\mathcal{L}(B))\le\gamma\cdot d$ .
